...
const webScrap = async (req, res) => {

   let deleteItem = await ScrapingDb.findAll()
   //console.log(deleteItem)
   if(deleteItem){
     const deleteData = await ScrapingDb.destroy({where:{}})
   }
	...
	try {
		...
		await page.waitForSelector('.css-j7qwjs');
		 ...
		 await browser.close();
       const data = await ScrapingDb.create({data:services})
